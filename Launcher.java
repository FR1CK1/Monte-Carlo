...
public class Launcher {
	...
	static double AREA = Math.PI * 0.5 * 0.5; // Kreisfl�che mit r = 1/2
	...
	public static void main(String[] args) {
		...
		double results[] = new double[INT_RESULT_STEP]; // Alle INT_RESULT Zwischenergebnise werden in dieses Array
														// geschrieben
		// F�r die Ermittlung, ob die Zufallszahlen im Bereich liegen oder nicht
		double randCount = 0;
		double randNotOK = 0;

		// Zufallsgenerator f�r x und y mit Pr�fung, ob Omega wahr ist
		System.out.println("Zuf�llige Koordinaten werden generiert und �berpr�ft...");
		for (int i = 0; i < N; i++) {
		...
		// Aus Effizientsgr�nden werden nur alle INT_RESULT Ergebnisse ermittelt
		...
		for (int i = 0; i < N; i++) {
			...
			if ((i + 1) % INT_RESULT == 0) {
				...
				resultPointer++;
				System.out.println(String.format("|%-15d|", (i + 1)) + String.format("%-25s|", integ));
			}
		}
		...
		System.out.println("Koordinaten au�erhalb des Bereiches Omega: " + (int) randNotOK);
		...
		Generator.saveRes(results); //�bertragung der Ergebnisse an andere Klassen
